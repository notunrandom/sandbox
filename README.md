...
Current exploration aims to introduce lazy decoding of CBOR-in-CBOR in [Block.hs][block-hs] 
...
- [x] Read documentation of `Codec.Serialise` and `Codec.CBOR` to find potential solutions
...
- However `unwrapCBORinCBOR` in [Block.hs][block-hs] uses its own `deserialise`, called `fromSerialised`.
- `Codec.CBOR.Encoding` (like CBOR itself) provides a way to write a list or bytes of indefinite length.

Solutions to explore:

- Use `Code.CBOR.Encoding.encodePreEncoded` (which seems to address the same situation as CBOR-in-CBOR...)
- Rewrite `deserialise` using `Control.Monad.ST.Lazy`
- Use `Codec.CBOR.Term`
- Keep CBOR-in-CBOR idea but encode already serialised bytestring using `toChunks`/`fromChunks` and `encodeIndef`/`encodeBreak`
